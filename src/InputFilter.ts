...
export default class InputFilter {
  ...
  public stringFilterSSC(this: any, input: string): Array<string> {
    ...
    for (var i = 0; i < betArr.length; i++) {
      if (
        /[^0-9]/.test(betArr[i]) || // 如果包含数字以外的内容，则去掉
        betArr[i].length !== this.betCount() || // 如果长度不一致，则去掉
        (this.noRepeat && Util.getDuplicate(betArr[i]) >= 2) || // 如果有重复去掉
        (this.isNeedRepeat && Util.getDuplicate(betArr[i]) < this.mustRepeatNum) || // 如果必须重复且小于重复次数去掉
        (this.noBaozi && Util.getDuplicate(betArr[i]) === this.betCount()) || // 有豹子去掉
